...
crc_t Utility::CalculateCRC32OfFile(string path)
{
    ...
    char databuf[bufflen];
    /* check file size and number of chunks to send */
    int fd = open(path.c_str(), O_RDONLY);
    if (fd < 0)
    {
        char err_msg[bufflen];
        snprintf(err_msg, bufflen, "Error while opening %s", path.c_str());
        perror(err_msg);
        exit(EXIT_FAILURE);
    }
    unsigned int length;
    crc_t crc = crc_init();
    while ((length = read(fd, databuf, bufflen)) > 0)
        crc = crc_update(crc, databuf, length);

    close(fd);
    return crc_finalize(crc);
}

crc_t Utility::crc_finalize(crc_t crc)
{
    return crc ^ 0xffffffff;
}

crc_t Utility::crc_init()
{
    return 0xffffffff;
}

crc_t Utility::crc_update(crc_t crc, const char* data, size_t data_len)
{
